...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            search = message.text
            search_ = re.sub(
                r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|dub(bed)|!|netflix|:|-|series|bluray|latest|season(s)*|episode(s)*|full\smovie|any(one)|with\ssubtitle(s)?)",
